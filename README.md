...
4. End of class: a study bundle (transcript + translations + AI study pack) downloads to a companion iOS app.
...
[USB mic] -> whisper.cpp (Pi) -> Gemma 4 E2B via Ollama (Pi) -> SSE -> student PWAs
                                              ...
                                              |
                                              v
                               iOS app (Gemma 4 E2B LiteRT, MTP)
...
- `mobile-app/` — Flutter iOS app for offline lecture Q&A

## Hardware

- **Pi-side:** Raspberry Pi 5 (8 GB), USB microphone, WiFi
- **Student-side during class:** any device with a browser
- **Student-side after class:** recent iPhone (~6 GB RAM, for on-device Gemma 4 E2B)
...
- [Gemma 4 E2B](https://huggingface.co/google/gemma-4-E2B) MTP-enabled `.litertlm` bundle on iOS via [flutter_gemma](https://pub.dev/packages/flutter_gemma) / [MediaPipe LLM Inference](https://ai.google.dev/edge/mediapipe/solutions/genai/llm_inference)
